...
def plot_unknown(f, filename):
    ...
    p = subprocess.Popen(['gnuplot -persist'], stdin=subprocess.PIPE,
            shell=True)
    #p.stdin.write('set nokey;')
    p.stdin.write('set key outside;')    #for showing legend
    p.stdin.write("set title 'Type=Unknown  File=%s';" % filename)
    command = ['plot ']
    for i in xrange(columns):
        #command.append("'%s' u %d w l," % (filename, i + 1))
    
        command.append("'%s' u %d w l lw 1 title '%d'," % (filename, i + 1, i + 1))
        #command.append("'%s' u %d w l lw 5 title '%d'," % (filename, i + 1, i + 1))  #plot thicker lines
