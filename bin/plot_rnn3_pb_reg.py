...
class plot_rnn(object):
    def __init__(self,
        ...
        self.figure_name = self.state_filename.replace(".log", ".pdf")
    ...
    def state(self, slice_num, tmin, tmax, use_colormap, width, height):
        ...
        var_state = self.data[:, self.in_state_size * 2:self.in_state_size * 3] + 0.00001
        ...
        likelihood = 0.5 * np.log(2 * np.pi * var_state[:-delay_length, :]) + 0.5 * (in_state[delay_length:, :] - out_state[:-delay_length, :]) * (in_state[delay_length:, :] - out_state[:-delay_length, :]) / var_state[:-delay_length, :]
        ...
        self.add_info(axes[5], None, (0, in_state.shape[0]), None , "Time step", "- log likelihood")
    ...
    def plot_phase(self, phase_plot_num, ax, state, label):
        ...
        self.add_info(ax, label, (-1.2,1.2), (-1.2,1.2), "2*i-th dim.", "(2*i+1)-th dim.")
    ...
    def phase(self, phase_plot_num, tmin, tmax, width, height):
        ...
        out_state = data[:, self.in_state_size: self.in_state_size+8]#2 * self.in_state_size
        ...
        gs.tight_layout(fig, rect = [0, 0, 1, 1]) # left, bottom, right, top
        fig.savefig(self.figure_name.replace(".pdf", "_phase.pdf"))
        fig.show()
