...
#librunner = cdll.LoadLibrary("/home/yiwen/rnn_pb/src/python/.libs/librnnrunner.so")
#librunner = cdll.LoadLibrary("/home/yiwen/rnn_pb_noassign/src/python/.libs/librnnrunner.so")
#librunner = cdll.LoadLibrary("/home/yiwen/rnn_pb_assign_old/src/python/.libs/librnnrunner.so")
librunner = cdll.LoadLibrary("/home/yiwen/rnn_pb_assign/src/python/.libs/librnnrunner.so")
#librunner = cdll.LoadLibrary("/home/yiwen/rnn_pb_converge/src/python/.libs/librnnrunner.so")

#librunner = cdll.LoadLibrary("/home/yiwen/rnn_mtrnn/src/python/.libs/librnnrunner.so")
#librunner = cdll.LoadLibrary("/home/yiwen/rnn_mtrnn_threshold/src/python/.libs/librnnrunner.so")
...
librunner.rnn_var_state_from_runner2.restype = POINTER(c_double)
librunner.update_rnn_runner2.argtypes = [c_void_p, c_int, c_int, POINTER(c_double), c_int, c_double, c_double]
...
class RNNRunner(object):
    ...
    def update(self, sequence, time_step, in_state, reg_count, rho_init, momentum):
        if in_state != None:
            ...
            x = None
        self.librunner.update_rnn_runner2(self.runner, sequence, time_step, x,
                                          reg_count, rho_init, momentum)
    ...
    def var_state(self, var_state=None):
        x = self.librunner.rnn_var_state_from_runner2(self.runner)
        if var_state != None:
            for i in xrange(len(var_state)):
                x[i] = c_double(var_state[i])
        return [x[i] for i in xrange(self.out_state_size())]
