...
//parameter controlling level of sensory variance K = -8, -4, 0, 4, 8
#ifndef K
#define K 0
#endif
        ...
        int *has_connection)
{
    ...
    foreach (i, connection) {      //for each elements i in connection
        has_connection[i] = 1;
    }
}
...
static double get_error_for_standard2 (const struct rnn_state *rnn_s)
{
    double error = 0;
    for (int n = 0; n < rnn_s->length; n++) {
        for (int i = 8; i < rnn_s->rnn_p->out_state_size; i++) {
            double d = rnn_s->out_state[n][i] - rnn_s->teach_state[n][i];
            error += 0.5 * d * d;
        }
    }
...
static double get_error_for_softmax2 (const struct rnn_state *rnn_s)
{
    double error = 0;
    for (int n = 0; n < rnn_s->length; n++) {
        for (int i = 8; i < rnn_s->rnn_p->out_state_size; i++) {
...
double rnn_get_error2 (const struct rnn_state *rnn_s)
{
    double error;
    if (rnn_s->rnn_p->output_type == STANDARD_TYPE) {
        error = get_error_for_standard2(rnn_s);
    } else if (rnn_s->rnn_p->output_type == SOFTMAX_TYPE){
        error = get_error_for_softmax2(rnn_s);
    } else {
        error = 0;
    }
    return error;
...
void rnn_forward_pb_assign_map(
       struct recurrent_neural_network *rnn)
{
    const int c_state_size = rnn->rnn_p.c_state_size;
    const int pb_num = rnn->rnn_p.pb_num;
    ...
    for(int i = 0; i < rnn->series_num; i++){
      ...
      if (pb_num == 2){
	if (i < 3){
	  rnn_s->init_c_inter_state[c_state_size-pb_num] = 1;
	  rnn_s->init_c_inter_state[c_state_size-pb_num+1] = 1;
	}
	else if (i < 6){
	  rnn_s->init_c_inter_state[c_state_size-pb_num] = 1;
	  rnn_s->init_c_inter_state[c_state_size-pb_num+1] = -1;
	...
	for(int j = 0; j < pb_num; j++){
	  if (i/3 == j){
	    rnn_s->init_c_inter_state[c_state_size-pb_num+j] = 1;
	  }
	  else{
	    rnn_s->init_c_inter_state[c_state_size-pb_num+j] = -1;
	  }
        ...
        double *var_state)
{
	...
	if(i < 8){//simulate aberrant variance
	  var_state[i] = exp(v_inter_state[i]);
	}else{
	  var_state[i] = exp(v_inter_state[i] + K);//K = -8, -4, 0, 4, 8
	}
    }
}
...
static void set_likelihood_for_standard2 (struct rnn_state *rnn_s)
{
    const struct rnn_parameters *rnn_p = rnn_s->rnn_p;
    for (int n = 0; n < rnn_s->length; n++) {
        for (int i = 0; i < rnn_p->out_state_size; i++) {
	  ...
	  if (i<8){
	    d = 0.0;
            ...
            d = rnn_s->teach_state[n][i] - rnn_s->out_state[n][i];
	  }
	  double s = 1.0 / (rnn_s->var_state[n][i] + MIN_VARIANCE);
	  rnn_s->delta_likelihood[n][i] = d * s;
	  rnn_s->likelihood[n][i] = -d * d * s;
...
void rnn_set_likelihood2 (struct rnn_state *rnn_s)
{
    if (rnn_s->rnn_p->output_type == STANDARD_TYPE) {
        set_likelihood_for_standard2(rnn_s);
    } else if (rnn_s->rnn_p->output_type == SOFTMAX_TYPE) {
        set_likelihood_for_softmax(rnn_s);
    }
}
...
static void backward_output_map_for_standard2(
	const struct rnn_parameters *rnn_p,
	const double *delta_likelihood,
	const double *out_state,		     
        const double *var_state, 	      
...
{
    if (rnn_p->output_type == STANDARD_TYPE) {
        backward_output_map_for_standard2(rnn_p, delta_likelihood, out_state,
                var_state, delta_o_inter, delta_v_inter);
    } else if (rnn_p->output_type == SOFTMAX_TYPE) {
        backward_output_map_for_softmax(rnn_p, delta_likelihood, out_state,
                delta_o_inter);
...
void rnn_backward_map2 (
        ...
        double *delta_v_inter)
{
    rnn_backward_output_map2(rnn_p, delta_likelihood, out_state, var_state,
            delta_o_inter, delta_v_inter);
    rnn_backward_context_map(rnn_p, delta_o_inter, delta_v_inter,
            next_delta_c_inter, c_state, delta_c_inter);
}
...
void rnn_backward_dynamics2 (struct rnn_state *rnn_s)
{
    const struct rnn_parameters *rnn_p = rnn_s->rnn_p;

    rnn_backward_map2(rnn_p, rnn_s->delta_likelihood[rnn_s->length-1], NULL,
            rnn_s->c_state[rnn_s->length-1], rnn_s->out_state[rnn_s->length-1],
            rnn_s->var_state[rnn_s->length-1],
            rnn_s->delta_c_inter[rnn_s->length-1],
            rnn_s->delta_o_inter[rnn_s->length-1],
    ...
    for (int n = rnn_s->length-2; n >= 0; n--) {
        rnn_backward_map2(rnn_p, rnn_s->delta_likelihood[n],
                rnn_s->delta_c_inter[n+1], rnn_s->c_state[n],
                rnn_s->out_state[n], rnn_s->var_state[n],
                rnn_s->delta_c_inter[n], rnn_s->delta_o_inter[n],
                rnn_s->delta_v_inter[n]);
...
void rnn_update_delta_init_c_inter_state2 (
        struct rnn_state *rnn_s,
        double momentum,
	double gamma_bp,
	double gamma_top,
	const int series_num)
{
    const struct rnn_parameters *rnn_p = rnn_s->rnn_p;
    const int pb_num = rnn_p->pb_num;
    double distance[series_num];
    double difference[series_num][pb_num];
    int pb_select = 0;
    ...
    for (int i = 0; i < series_num; i++){
      for (int j = 0; j < pb_num; j++){
	difference[i][j] = (rnn_s->init_c_inter_state[j + rnn_p->c_state_size - pb_num] - rnn_s->assigned_pb[i][j]);
	distance[i] += difference[i][j] * difference[i][j];
      }
    ...
    for (int i = rnn_p->c_state_size-pb_num; i < rnn_p->c_state_size; i++) {
      rnn_s->delta_init_c_inter_state[i] = gamma_bp * rnn_s->delta_i[i] 
	+ gamma_top * difference[pb_select][i + pb_num - rnn_p->c_state_size]  
	+ momentum * rnn_s->delta_init_c_inter_state[i];
      
      //printf("value   ");
      //printf("%f   ", rnn_s->delta_init_c_inter_state[i]);
      //printf("%f    ", rnn_s->delta_i[i]);
      //printf("%f    ", difference[pb_select][i + pb_num - rnn_p->c_state_size]);
      //printf("%f    ", rnn_s->delta_init_c_inter_state[i]);
      //printf("\n");
